...
import { createAtaIfMissingInstruction, getAta, TOKEN_PROGRAM_ADDRESS } from "../spl.js";
...
function loadX25519Keypair(filePath: string): { secretKey: Uint8Array; publicKey: Uint8Array } {
  const parsed = JSON.parse(fs.readFileSync(filePath, "utf8")) as X25519KeypairJson;
  if (!Array.isArray(parsed.secretKey) || !Array.isArray(parsed.publicKey)) {
    throw new Error(`X25519 keypair file must contain secretKey and publicKey arrays: ${filePath}`);
  }
...
export function registerVouchCommands(program: Command): void {
  program
    ...
    .action(async (options, command) => {
      ...
      const amount = options.amount ? BigInt(options.amount) : await promptBigInt("Vouch amount", market.data.minVouchAmount);
      const optionId = options.optionId ? Number(options.optionId) : await promptNumber("Option ID", 0);
      const keypairPath = options.x25519Keypair ?? await promptOptionalString("X25519 keypair path (blank to generate ephemeral)");
      const x25519Keypair = keypairPath ? loadX25519Keypair(keypairPath) : generateX25519Keypair();
      const mxePublicKey = new Uint8Array(Buffer.from(await getMxePublicKeyHex(ctx), "hex"));

      const signerTokenAccount = await getAta(market.data.mint, ctx.payer.address);
      const createAtaIx = await createAtaIfMissingInstruction(ctx, market.data.mint, ctx.payer.address);
      const vouchAccountId = Math.floor(Math.random() * 1_000_000_000) + 1;
      const [vouchAccount] = await getVouchAccountAddress(ctx.payer.address, market.address, vouchAccountId, ctx.programId);
      ...
      const firstInstructions: Instruction[] = createAtaIx ? [createAtaIx, initIx as Instruction] : [initIx as Instruction];
      const initSig = await sendInstructions(ctx, firstInstructions, "init vouch account");
      printTxResult(initSig);

      const cipher = createCipher(x25519Keypair.secretKey, mxePublicKey);
      const inputNonce = randomBytes(16);
      const selectedOptionCiphertext = cipher.encrypt([BigInt(optionId)], inputNonce)[0];
      const computationOffset = randomComputationOffset();

      const vouchIx = await vouch(
        {
          programAddress: ctx.programId,
          signer: ctx.payer,
          payer: ctx.payer,
        ...
        {
          clusterOffset: Number(options.clusterOffset),
          computationOffset,
          programId: ctx.programId,
        },
      );
      const vouchSig = await sendInstructions(ctx, [vouchIx as Instruction], "vouch");
