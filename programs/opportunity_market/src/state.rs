...
impl PlatformConfig {
    pub fn try_new(
        ...
        #[cfg(not(feature = "disable-prod-guardrails"))]
        require!(
            min_time_to_vouch_seconds >= MIN_TIME_TO_VOUCH_FLOOR_SECONDS,
            ErrorCode::InvalidParameters
        );
        require!(
            min_time_to_vouch_seconds <= MAX_TIME_TO_VOUCH_SECONDS,
            ErrorCode::InvalidParameters
        );
        require!(
...
pub struct OpportunityMarket {
    ...
    // Public key for voluntary disclosure of encrypted vouch data
    ...
    // Minimum vouch amount (in SPL token base units) required for a vouch.
...
impl OpportunityMarket {
    pub fn phase(&self, now: u64) -> Result<MarketPhase> {
        ...
        let Some(vouching_window_end) = self.vouching_window_end else {
            return Ok(MarketPhase::NotOpen);
        };

        if now < vouching_window_end {
            return Ok(MarketPhase::Vouching);
        }

        let deadline = vouching_window_end
...
pub struct VouchAccount {
    ...
    // Computation account pubkey of the in-flight vouch computation.
    // `Some` means a vouch computation is pending; None means no vouch is in flight.
    pub pending_vouch_computation: Option<Pubkey>,
...
mod tests {
    ...
    const VOUCHING_END: u64 = 1_000;
    const RESOLUTION_DEADLINE_SECS: u64 = 600;
    const SELECTION_END: u64 = VOUCHING_END + RESOLUTION_DEADLINE_SECS;

    fn test_market(
        vouching_window_end: Option<u64>,
    ...
    #[test]
    fn phase_not_open_when_vouching_window_unset() {
    ...
    #[test]
    fn phase_selection_after_vouching_window() {
        let market = test_market(Some(VOUCHING_END), None, false);
        assert_eq!(market.phase(VOUCHING_END).unwrap(), MarketPhase::Selection);
        assert_eq!(
            market.phase(VOUCHING_END + 1).unwrap(),
    ...
    fn phase_revealing_when_resolved_before_reveal_ends() {
        let market = test_market(Some(VOUCHING_END), Some(VOUCHING_END + 1), false);
        assert_eq!(
            market.phase(VOUCHING_END + 1).unwrap(),
            MarketPhase::Revealing
        );
    ...
    fn phase_settlement_when_resolved_and_reveal_ended() {
        let market = test_market(Some(VOUCHING_END), Some(VOUCHING_END + 1), true);
        assert_eq!(
            market.phase(VOUCHING_END + 1).unwrap(),
            MarketPhase::Settlement
        );
    ...
    fn require_phase_helpers_reject_mismatch() {
        let market = test_market(Some(VOUCHING_END), None, false);
        assert!(market
            .require_phase(VOUCHING_END - 1, MarketPhase::Vouching)
            .is_ok());
        assert!(market
            .require_phase(VOUCHING_END, MarketPhase::Selection)
            .is_ok());
        assert!(market
            .require_phase(VOUCHING_END, MarketPhase::Vouching)
            .is_err());
        assert!(market
            .require_phase(VOUCHING_END + 1, MarketPhase::Vouching)
            .is_err());
        assert!(market
            .require_phase_at_least(VOUCHING_END + 1, MarketPhase::Revealing)
            .is_err());
        assert!(market
            .require_phase_at_most(VOUCHING_END - 1, MarketPhase::Vouching)
            .is_ok());
        assert!(market
            .require_phase_at_most(VOUCHING_END, MarketPhase::Vouching)
            .is_err());
        assert!(market
            .require_phase_at_most(VOUCHING_END + 1, MarketPhase::Vouching)
            .is_err());
