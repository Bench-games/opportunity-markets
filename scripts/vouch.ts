...
if (!MARKET_ADDRESS || !AMOUNT || !OPTION_ID) {
  console.error("Usage: npx tsx scripts/vouch.ts <MARKET_ADDRESS> <AMOUNT> <OPTION_ID> [X25519_KEYPAIR_PATH]");
  process.exit(1);
}
...
async function main() {
  ...
  console.log(`Vouching user ATA created: ${ataSig}`);
  }
  ...
  console.log(`\nInitializing vouch account (id: ${vouchAccountId})...`);
  ...
  console.log(`Init vouch account sig: ${initSig}`);

  const [vouchAccountAddress] = await getVouchAccountAddress(payer.address, marketAddress, vouchAccountId, PROGRAM_ID);
  ...
  console.log(`\nVouching ${amount} tokens on option ${optionId}...`);
  ...
  const signedVouchTx = await signTransactionMessageWithSigners(
    pipe(
      createTransactionMessage({ version: 0 }),
      (msg) => setTransactionMessageFeePayer(payer.address, msg),
      (msg) => setTransactionMessageLifetimeUsingBlockhash(latestBlockhash2, msg),
      (msg) => appendTransactionMessageInstructions([vouchIx as Instruction], msg)
    )
  );

  const vouchSig = await sendAndConfirmTx(rpc, signedVouchTx);
  console.log(`Done. Vouch account ID: ${vouchAccountId}`);
  console.log(`Signature: ${vouchSig}`);
}
